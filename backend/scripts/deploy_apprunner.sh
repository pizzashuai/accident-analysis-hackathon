...
# Deploy the backend API to AWS App Runner in us-east-2 (override via AWS_REGION).
# Relies on AWS CLI v2 and Docker being configured on the machine executing this script.
...
SERVICE_NAME="${SERVICE_NAME:-accident-api}"
ECR_REPO="${ECR_REPO:-accident-api}"
IMAGE_TAG="${IMAGE_TAG:-latest}"
CONTAINER_PORT="${CONTAINER_PORT:-8000}"
HEALTHCHECK_PATH="${HEALTHCHECK_PATH:-/api/v1/openapi.json}"
...
DOCKERFILE_PATH="${DOCKERFILE_PATH:-src/api/Dockerfile}"
