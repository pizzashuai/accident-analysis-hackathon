...
class VideoProcessor:
    ...
    def process_video_detections(
        ...
        logger.info(
            f"Using optimal smoothing: {self.bbox_smoothing_method} bbox + {self.speed_smoothing_method} speed + {self.tracking_point} tracking"
        )
        ...
        logger.info(
            f"Detection and tracking complete! Processed {processed_frames} frames"
        )
    ...
    def _process_detections_for_speed(
        ...
        for i in range(len(detections)):
            ...
            vehicle_positions[track_id].append(
                (frame_count, float(x_norm), float(y_norm), float(timestamp))
            )
            ...
            if len(history) >= 5:
                ...
                try:
                    ...
                    if time_diff > 0:
                        ...
                        if (
                            raw_speed_mph < min_reasonable_speed
                            or raw_speed_mph > max_reasonable_speed
                        ):
                        ...
                        speed_mph = self._speed_smoother.smooth_speed(
                            track_id, raw_speed_mph
                        )

                        # Final bounds check after smoothing
                        if (
                            speed_mph < min_reasonable_speed
                            or speed_mph > max_reasonable_speed
                        ):
                            if track_id in vehicle_speeds:
                                speed_mph = vehicle_speeds[
                                    track_id
                                ]  # Use previous speed
                            else:
                                speed_mph = min(
                                    max(speed_mph, min_reasonable_speed),
                    ...
                    logger.warning(
                        f"Failed to calculate speed for track {track_id}: {e}"
                    )
    ...
    def create_homography_data(
        ...
        for idx, pair in enumerate(homography_session.pairs):
            logger.info(
                f"Pair {idx}: image({pair.image_x_norm:.4f}, {pair.image_y_norm:.4f}) -> geo({pair.map_lat:.6f}, {pair.map_lng:.6f})"
            )
            pairs_data.append(
                {
                    "id": idx,
        ...
        homography_data = {
            "pairs": pairs_data,
            "imagesMeta": homography_model.meta.get("imagesMeta", {})
            if homography_model.meta
            else {},
            "mapMeta": homography_model.meta.get("mapMeta", {})
            if homography_model.meta
            else {},
        }
    ...
    def process_video_detections_from_objects(
        ...
        for frame_count, frame in enumerate(video_frames):
            ...
            class_names = (
                getattr(getattr(self._model, "model", None), "names", {}) or {}
            )

            for di in range(len(detections)):
                # tracker id
                tid = (
                    None if detections.tracker_id is None else detections.tracker_id[di]
                )
                # bbox
    ...
    def convert_detections_to_database_format_from_objects(
        ...
        if homography_data:
            ...
            logger.info(
                f"Initialized DistanceEstimator with {len(distance_estimator.homography_data.pairs)} point pairs"
            )
        ...
        for detection in detections_list:
            ...
            if track_id is not None:
                ...
                # Apply Kalman smoothing to bbox
                if len(vehicle_bboxes[track_id]) >= 2:
                    smoothed_bbox = bbox_smoother._kalman_smooth(
                        track_id, np.array(bbox)
                    )
            ...
            if track_id is not None and distance_estimator:
                ...
                vehicle_positions[track_id].append(
                    (detection["frame"], float(x_norm), float(y_norm), float(timestamp))
                )
                ...
                if len(history) >= 5:
                    ...
                    try:
                        ...
                        if time_diff > 0:
                            ...
                            # Apply speed smoothing (now using Kalman with outlier rejection)
                            speed_mph = speed_smoother.smooth_speed(
                                track_id, raw_speed_mph
                            )
            ...
            detection_data = {
                ...
                "extra": {
                    ...
                    "smoothed_bbox": [
                        float(x) for x in smoothed_bbox
                    ],  # Convert numpy array to Python list
        ...
        speed_count = sum(
            1 for d in detections_data if d["extra"].get("speed_mph") is not None
        )
    ...
    def update_jsonl_with_speed_data(
        ...
        with (
            open(jsonl_path) as input_file,
            open(updated_jsonl_path, "w") as output_file,
        ):
            for line in input_file:
                ...
                for det_data in detections_data:
                    if det_data["frame_idx"] == detection["frame"] and det_data[
                        "track_id"
                ...
                if (
                    matching_detection
                    and matching_detection["extra"].get("speed_mph") is not None
                ):
                    ...
                    if speed_updates_count <= 5:
                        logger.info(
                            f"Updated detection frame {detection['frame']} track {detection.get('track_id')} with speed {detection['speed_mph']:.2f} mph"
                        )

                output_file.write(json.dumps(detection) + "\n")

        logger.info(
            f"JSONL update complete: {speed_updates_count}/{total_detections} detections updated with speed data"
        )
    ...
    def create_annotated_video(
        ...
        """
        logger.info(
            f"Using optimal smoothing configuration: Kalman filter for bbox (22.2% improvement) + Kalman with outlier rejection for speed (max speed: {self.max_reasonable_speed} mph) + Bottom center tracking"
        )
...
def analyze_speeds_from_detections(
    ...
    distance_estimator = DistanceEstimator(homography_file)
    logger.info("Initialized DistanceEstimator with homography data")
    ...
    if all_raw_speeds:
        stats = {
            ...
            "outliers_count": sum(
                1 for s in all_raw_speeds if s > max_reasonable_speed
            ),
            "outliers_percent": 100
            * sum(1 for s in all_raw_speeds if s > max_reasonable_speed)
            / len(all_raw_speeds),
        ...
        logger.info(
            f"  Outliers (>{max_reasonable_speed} mph): {stats['outliers_count']} ({stats['outliers_percent']:.1f}%)"
        )
...
def calculate_speeds_with_smoothing(
    ...
    for det in detections:
        ...
        if len(vehicle_positions[track_id]) > lookback_frames * 2:
            vehicle_positions[track_id] = vehicle_positions[track_id][
                -lookback_frames * 2 :
            ]
                ...
                if time_diff > 0:
                    ...
                    if smoothing_method == "median_moving_average":
                        ...
                        if len(recent_speeds) >= 3:
                            ...
                            if (
                                abs(raw_speed_mph - median_speed)
                                > max_reasonable_speed * 0.5
                            ):
                    ...
                    elif smoothing_method in [
                        "kalman_with_outlier_rejection",
                        "exponential_with_outlier_rejection",
                    ]:
                        ...
                        if (
                            ...
                            speed_mph = np.clip(
                                speed_mph, min_reasonable_speed, max_reasonable_speed
                            )
                            stats["outliers_rejected"] += 1
    ...
    if stats["smoothed_speeds"]:
        ...
        stats["outlier_rejection_rate"] = (
            100 * stats["outliers_rejected"] / stats["speed_calculations"]
        )
    ...
    logger.info("Speed calculation complete:")
    logger.info(
        f"  Speeds assigned: {stats['speeds_assigned']}/{stats['total_detections']}"
    )
    logger.info(
        f"  Outliers rejected: {stats['outliers_rejected']} ({stats.get('outlier_rejection_rate', 0):.1f}%)"
    )
    logger.info(f"  Avg smoothed speed: {stats.get('avg_smoothed_speed', 0):.2f} mph")
    logger.info(
        f"  Median smoothed speed: {stats.get('median_smoothed_speed', 0):.2f} mph"
    )
    logger.info(
        f"  Speed range: {stats.get('min_smoothed_speed', 0):.2f}-{stats.get('max_smoothed_speed', 0):.2f} mph"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process video detections and calculate speeds"
    )
    parser.add_argument(
        "--jsonl", type=str, required=True, help="Path to detections JSONL file"
    )
    parser.add_argument(
        "--homography", type=str, required=True, help="Path to homography JSON file"
    )
    parser.add_argument(
        "--output-dir", type=str, default="./speed_test_output", help="Output directory"
    )
    parser.add_argument(
        "--video-width", type=int, default=1280, help="Video width in pixels"
    )
    parser.add_argument(
        "--video-height", type=int, default=720, help="Video height in pixels"
    )
    parser.add_argument(
        "--max-speed", type=float, default=100.0, help="Maximum reasonable speed in mph"
    )
    parser.add_argument(
        "--min-speed", type=float, default=0.0, help="Minimum reasonable speed in mph"
    )
    parser.add_argument(
        "--lookback", type=int, default=5, help="Lookback frames for speed calculation"
    )
    ...
    for method, result in results.items():
        ...
        logger.info(
            f"{method:<35} {avg_speed:>12.2f} {median_speed:>12.2f} {max_speed:>12.2f} {outliers:>12}"
        )

    logger.info(f"\n{'=' * 80}")
    logger.info(
        "RECOMMENDATION: Use 'median_moving_average' for best outlier rejection"
    )
    logger.info(f"{'=' * 80}")
