                ...
                summary_data.append(
                    {
                        ...
                        "IoU": f"{row.iou:.3f}",
                        "Distance (m)": f"{row.world_distance_m:.1f}"
                        if row.world_distance_m
                        else "N/A",
                        "Speed Diff (m/s)": f"{row.relative_speed_mps:.1f}"
...
def _generate_narrative(
    ...
    """Generate narrative text for a timeline stage."""
    if stage == "approach":
        return f"Vehicles approaching with {row.world_distance_m:.1f}m separation"
    elif stage == "first_contact":
        return f"First contact detected with IoU {row.iou:.3f}"
    elif stage == "peak_overlap":
        return f"Maximum overlap with IoU {row.iou:.3f}"
    elif stage == "separation":
        return "Vehicles separating after collision"
    else:
        return f"Stage {stage} at frame {row.frame}"
